...
namespace RealmGames.TileSystem
{
    public class TileRenderer : MonoBehaviour
    {
        ...
        public void UpdateTexture(int x, int y)
        {
            ...
            byte map_value = m_gameBoardDefinition.GetMapValue(x, y);

            if(map_value == 1)
            {
                gameBoard.SetBackgroundSprite(x, y, tileset.GetSprite("default"));
                gameBoard.SetSlotSprite(x, y, null);
            }

            if (map_value == active_color)
            {
